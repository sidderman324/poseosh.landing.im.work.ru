		...
		if ( $this->archive ) {
			...
			switch ( $this->type ) {
				case WPS_TYPE_AUTHOR :
					...
					break;
				case WPS_TYPE_CAT :
					$join =  "JOIN $wpdb->term_relationships ON posts.ID = $wpdb->term_relationships.object_id JOIN $wpdb->term_taxonomy ON $wpdb->term_taxonomy.term_taxonomy_id = $wpdb->term_relationships.term_taxonomy_id";
					$where = "$wpdb->term_taxonomy.taxonomy = 'category' AND $wpdb->term_taxonomy.term_id in (". implode(',', $this->getAllIDs()) .")";
					break;
				default :
					break;
			}
	...
	function getTiedPages() {
		...
		if ( get_option( WPS_OPT_PAGEFILTER ) ) {
			if ( ! $this->tied_pages ) {
				...
				$args = array(
					'meta_key' => $wps_page_metakey_tie,
					'meta_value' => $this->id
				);
				$this->tied_pages = get_posts( $args );
